...
프론트엔드 개발자는 백엔드 담당자에게 별도로 전달받은 `.env` 파일을 프로젝트 루트에 두고 실행합니다.
...
MySQL 컨테이너를 최초 실행하면 아래 누적 DDL 파일이 자동 실행됩니다.

```text
src/main/resources/db/DDL_V9.sql
```

그리고 프론트엔드 로컬 연동 테스트에 필요한 기본 약관 데이터도 함께 적재됩니다.
...
즉, `mysql-data` 볼륨이 이미 생성되어 있으면 DDL은 다시 실행되지 않습니다.
...
로컬에서 아직 S3 버킷을 만들지 않았다면 이 값은 임시값으로 둘 수 있습니다. 다만 프로필 이미지 업로드 URL 발급, 실제 S3 업로드, 업로드 확정 API까지 테스트하려면 유효한 S3 버킷, AWS 인증 정보, 버킷 권한이 필요합니다.
