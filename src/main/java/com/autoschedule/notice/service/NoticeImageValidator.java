...
/**
 * 공지 이미지 업로드 메타데이터와 실제 S3 객체의 이미지 여부를 검증한다.
 */
@Component
public class NoticeImageValidator {
    ...
    public NoticeImageValidator(NoticeImageProperties properties) {
    ...
    /**
     * S3 업로드 완료 후 실제 객체 크기, content type, 매직 바이트를 검증한다.
     */
    public void validateActualImage(String contentType, long fileSize, byte[] firstBytes) {
        ...
        if (!matchesMagicBytes(normalizedContentType, firstBytes)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "업로드된 파일이 올바른 공지 이미지 파일이 아닙니다.");
        }
    }
    ...
    public String normalizeContentType(String contentType) {
        if (!StringUtils.hasText(contentType)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 content type은 필수입니다.");
        }
    ...
    public String extractExtension(String originalFileName) {
        if (!StringUtils.hasText(originalFileName)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 원본 파일명은 필수입니다.");
        }
        if (containsPathOrControlCharacter(originalFileName)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 원본 파일명에는 경로 문자나 제어 문자를 포함할 수 없습니다.");
        }
        int dotIndex = originalFileName.lastIndexOf('.');
        if (dotIndex < 0 || dotIndex == originalFileName.length() - 1) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 파일 확장자가 올바르지 않습니다.");
        }
    ...
    public void validateImageCount(int imageCount) {
        if (imageCount > properties.maxCount()) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지는 최대 %d장까지 첨부할 수 있습니다.".formatted(properties.maxCount()));
        }
    }
    ...
    private void validateContentType(String contentType) {
        if (!ALLOWED_CONTENT_TYPES.contains(contentType)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지는 JPG, PNG, WEBP 형식만 업로드할 수 있습니다.");
        }
    }

    /**
     * 파일 크기가 1 byte 이상, 정책상 최대 크기 이하인지 확인한다.
     */
    private void validateFileSize(long fileSize) {
        if (fileSize <= 0) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 파일 크기는 1 byte 이상이어야 합니다.");
        }
        if (fileSize > properties.maxSizeBytes()) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 파일 크기는 최대 10MB까지 허용됩니다.");
        }
    }

    /**
     * 파일 확장자와 content type 조합이 일치하는지 확인한다.
     */
    private void validateFileName(String originalFileName, String contentType) {
        ...
        if (!ALLOWED_EXTENSIONS.getOrDefault(contentType, Set.of()).contains(extension)) {
            throw new ApiException(ErrorCode.INVALID_REQUEST, "공지 이미지 파일 확장자와 형식이 일치하지 않습니다.");
        }
    }
